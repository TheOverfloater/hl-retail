...
// first try just moving to the destination	
	...
	if (trace.fraction == 1)
	{
		...
		// Try and move down the stairs
		if(!pmove->waterjumptime && oldonground != -1)
		{
			...
			trace = pmove->PM_PlayerTrace(pmove->origin, dest, PM_NORMAL, -1 );
			if( trace.fraction < 1.0 && trace.fraction > 0.0 && trace.plane.normal[2] > 0.7 
...
int PM_CheckStuck (void)
{
	...
	for(i = 0; i < 8; i++)
	{
		for(j = 0; j < 8; j++)
		{
			for(k = 0; k < 8; k++)
			{
				test[0] = pmove->origin[0]+i;
				test[1] = pmove->origin[1]+j;
				test[2] = pmove->origin[2]+k;
