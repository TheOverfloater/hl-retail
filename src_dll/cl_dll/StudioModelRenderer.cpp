...
void CStudioModelRenderer::StudioEntityLight ( void )
{
	...
	for(unsigned int i = 0; i < m_iNumEntityLights; i++)
	{
		...
		if(!plight->temporary)
		{
			...
			if(plight->origin[0] > m_vMaxs[0] || plight->origin[1] > m_vMaxs[1] || plight->origin[2] > m_vMaxs[2]
				|| plight->origin[0] < m_vMins[0] || plight->origin[1] < m_vMins[1] || plight->origin[2] < m_vMins[2])
			{
