...
public:
	...
	// Draw glow shells
	virtual void StudioRenderGlowShell (void);
	...
	// Gets lighting information for model
	virtual void StudioDynamicLight( void );
...
public:
	// Sets up normals for glow shell rendering
	virtual void StudioSetupGlowShellNormals( void );

	// Draws glow shell for a submodel
	virtual void StudioDrawGlowShell( void );
...
public:
	...
	// Gathered normal vectors for vertexes
	vec3_t			m_vertexNormals[MAXSTUDIOVERTS];
	// Number of normals gathered for each vertex
	int				m_vertexNumNormals[MAXSTUDIOVERTS];
	// Chrome origin
