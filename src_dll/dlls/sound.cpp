...
{
	// Treat this as a trigger volume
	pev->solid		= SOLID_TRIGGER;
	pev->movetype	= MOVETYPE_NONE;
	pev->effects	|= EF_NODRAW;

	UTIL_SetSize(pev, Vector(-m_flRadius, -m_flRadius, -m_flRadius), Vector(m_flRadius, m_flRadius, m_flRadius));
}
...
void CEnvSound :: Touch( CBaseEntity *pOther )
{
	...
	Vector vecSpot2 = pOther->pev->origin + pOther->pev->view_ofs;
	TraceResult tr;
	...
	float dist = (vecSpot1 - vecSpot2).Length();
	if((pPlayer->m_flLastClosestDistance == 0 
		|| pPlayer->m_flLastClosestDistance > dist)
		&& pPlayer->m_flSndRoomtype != m_flRoomtype)
	{
		// Tell player to use this sound type
		pPlayer->m_flSndRoomtype = m_flRoomtype;
		pPlayer->m_flLastClosestDistance = dist;
	}
	else if(pPlayer->m_flSndRoomtype == m_flRoomtype)
	{
		pPlayer->m_flLastClosestDistance = dist;
	}
}
